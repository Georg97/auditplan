...
- [ ] S08 [SETUP] Bestehende Demo-Inhalte entfernen — +page.svelte Demo-Code und data.remote.ts Beispieldaten entfernen, console.log-Statements entfernen, @ts-ignore in hooks.server.ts durch korrekte Typen ersetzen, Root-Seite durch Redirect zu /overview ersetzen — Specs: 01-architektur
- [ ] S09 [SETUP] Server-Funktionen-Scaffolding — Remote-Functions-Dateien (.remote.ts) mit query()/mutation() Stubs für Auditoren, Audits, Kalender, Maßnahmen, Pläne, Notizen, Auditfragen, Einstellungen — Specs: 01-architektur
